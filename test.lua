...
assert(not pcall(fun),
  "Conversation.listen should throw an error if the second argument isn't a function")
...
assert(arbitraryNumber == 5,
  'conversation.say should be triggering the listener that was just set up')
assert(a == 5 and b == 7 and c == 10,
  'The listeners should be returning values through conversation.say')
...
assert(not (d or e or f),
  'All of the listeners in the group should have been disabled')
